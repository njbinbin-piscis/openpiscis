...
impl CallKoiTool {
    ...
    async fn call_koi(&self, input: &Value, ctx: &ToolContext) -> anyhow::Result<ToolResult> {
        ...
        let (koi_def, pool_session_id) = {
            ...
            (koi_def, pool_session.map(|session| session.id))
        };
        ...
        let system_prompt =
            assemble_koi_task_system_prompt(&state, &koi_def, pool_session_id.as_deref(), &task)
                .await;
        tracing::info!(
            "koi_context_slices koi={} pool={} history_mode={:?} memory_mode={:?} event_digest_mode={:?} system_prompt_chars={}",
            koi_id,
            pool_session_id.as_deref().unwrap_or("default"),
