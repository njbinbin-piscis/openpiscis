...
**权限设置即时生效** —— 在设置中关闭 Shell/文件写入确认后，正在运行的会话也会立即生效，无需重开对话。并修复悬浮窗、设置、调试等国际化，以及 Windows PPTX 读取。

## 🕘 v0.8.52 — 鱼池项目同步

主会话创建的项目会实时出现在鱼池侧栏；切回鱼池 Tab 自动刷新，并提供 ↻ 手动刷新按钮。

## 🕘 v0.8.51 — 输入历史

主会话、鱼池聊天室、IDE CLI 三个输入框支持 ↑/↓ 恢复历史发送内容，各自独立记录。

## 🕘 v0.8.50 — 技能选择优化

对话框技能下拉仅显示用户安装的技能；系统内置技能仍通过 `skill_list` 开放。

## 🕘 v0.8.49 — CI 发布修复

v0.8.47/48 无安装包；本版起 CI 正常产出安装包。功能同 v0.8.48。

### 🎨 界面（v0.8.48 起）
- **`AppDropdown`**：主会话工作区/锦鲤/技能、看板筛选、审计日志工具筛选统一为可保持展开的弹出列表。
- **锦鲤人设**：选择锦鲤后多轮对话保持，直到手动清除。
- **鱼池参与者**：⚙ 齿轮打开居中模态（不再挤占聊天区域）。

## 🕘 v0.8.48 / v0.8.47 — 无安装包（CI fmt 失败）

请使用 **v0.8.49** 或 **v0.8.46** 下载安装包。

## 🕘 v0.8.46 — RobotZ 与 MCP UI

RobotZ 内置工具定位澄清；MCP 传输下拉与鱼池 Git 图标统一。

## 🕘 v0.8.45 — UI 与微信 IM

深浅主题、紧凑控件、微信发文件提示词、鱼池 i18n 与 placeholder 修复。

## 🕘 v0.8.44 — 鱼池 CLI 仅 IDE 创建
