...
- **Pisci 个性化提示词**：新增全局提示词设置，仅作用于 Pisci 主会话、heartbeat、Pool 协调与定时任务会话，不影响 Koi 和 Fish。
- **会话 Artifacts 面板**：普通聊天会话的 Task Panel 新增 Artifacts 标签，与 Todo、Tools 并列，用于汇总当前会话生成的成果。
- **成果提交工具**：Pisci 可通过 `app_control` 提交生成的文件或 URL，让成果既能在消息气泡中作为链接查看，也能在 Artifacts 面板中统一浏览。
- **运行时命令处理**：改进系统/运行时命令处理与桌面自动化诊断路径。
- **调试界面刷新**：更新屏幕诊断相关 UI，让自动化排查时的信息更清晰。
