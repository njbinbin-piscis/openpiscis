...
- **微信入站媒体现在会落成真实文件**：来自 iLink 网关的图片、语音、文件和视频会从微信 CDN 下载并在本地解密，然后以真实附件字节交给 Pisci，而不再只是占位提示。
- **微信出站媒体兼容性更稳**：出站 CDN 媒体负载现在对齐 iLink SDK 的 `aes_key` 编码格式，减少微信客户端把附件显示成“已过期”或“已被清理”的情况。
- **中断流式回复时会保留已生成内容**：如果用户在流式输出已经返回部分文本后取消本轮回复，Pisci 现在会把这段已生成的 assistant 文本保存在会话历史里，而不是直接丢掉。
