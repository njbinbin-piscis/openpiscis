...
- **显式 IM 渠道工具**：Agent 现在可以显式列出已配置 IM 渠道、按需发起连接，并解析渠道 binding，而不是依赖隐式路由猜测。
- **定时任务到 IM 的上下文连续性**：定时任务通知发送成功后，会在需要时自动创建缺失的 IM 会话，并把出站消息镜像写入目标 IM 历史，后续回复可以继承完整上下文。
- **更稳定的定时投递**：定时任务在重启恢复和增删改后会去重 cron 注册，避免同一任务被重复恢复、多次触发。
- **更顺手的桌面交互**：聊天页工作区切换会立即显示刚选中的目录，流式输出在分段和结束时会正确刷新缓存增量，后台连接成功后 IM 连接状态也会立刻同步到界面。
- **设置页整理**：设置页中的模型供应商文案更清晰，同时将默认 heartbeat 提示词改为内建字符串，避免模块初始化阶段依赖 i18n。
